...
if (TYPE == "pwlu" or TYPE == "nupwlu") and STATISTICS:
	h.PWLU_Set_StatisticalToggle(efficient_net_v2_s, "ShuffleBlock", True)
	print(f"\t\tBeginning Statistical Analysis")
	tme = time.time()
	for x, _ in train_ds:
		efficient_net_v2_s.call(x)
	print(f"\t\tStatistics took {time.time() - tme}s to calculate")
	h.PWLU_Set_StatisticalToggle(efficient_net_v2_s, "ShuffleBlock", False)
