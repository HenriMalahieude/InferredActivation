...
with strat.scope():
	...
	efficient_net_v2_s = models.Sequential([
		...
		layers.BatchNormalization(-1),
		act_to_use(act_arg),
		*[MBConv(1, 3, 24, stride=1, version="fused") for _ in range(2)], #2 relus * 2 layers
		*[MBConv(4, 3, 48, stride=2, version="fused") for _ in range(4)], #1 relus * 4 layers
		*[MBConv(4, 3, 64, stride=2, version="fused") for _ in range(4)], #1 relus * 4 layers

		*[MBConv(4, 3, 128, se_factor=0.25, stride=2) for _ in range(6)], #(3 relus + 1 sig) * 6 layers
		*[MBConv(6, 3, 160, se_factor=0.25) for _ in range(9)],			  #(3 relus + 1 sig) * 9 layers
		*[MBConv(6, 3, 256, se_factor=0.25, stride=2) for _ in range(15)],#(3 relus + 1 sig) * 15 layers
